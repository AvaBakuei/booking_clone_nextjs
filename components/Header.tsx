...
import {
    ...
    HomeIcon,
    PaperAirplaneIcon,
    PhoneIcon, PlayCircleIcon, XMarkIcon
...
function Header() {
    ...
    return(
        <header className="bg-[#013894]">
            <nav className="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-global="Global">
                ...
                <Popover.Group className="hidden lg:flex lg:gap-x-12">
                    <Popover className="relative">
                        <Popover.Button className="flex items-center gap=x-1 text-sm font-semibold leading-6 text-white">
                            Stays
                            <ChevronDownIcon className="h-5 w-5 flex-none tetx-white" aria-hidden="true" />
                        </Popover.Button>
                            ...
                            <Popover.Panel className="absolute bg-white -left-8 top-full z-10 mt-3 w-screen max-w-md overflow-hidden rounded-3xl shadow-lg ring-1 ring-gray-900/5">
                                <div className="p-4">
                                    {products.map((item)=>(
                                        <div key={item.name} className="group relative flex items-center gap-x-6 rounded-lg p-4 text-sm leading-6 hover:bg-gray-50">
                                            <div className="flex items-center justify-center w-11 h-11 flex-none items-center rounded-lg bg-gray-50 group-hover:bg-gray-200">
                                ...
                                <div className="grid grid-cols-2 divide-gray-900/5 bg-gray-50">
                                    {callsToAction.map((item)=>(
                                        <a key={item.name}
                                           href={item.href}
                                           className="flex items-center justify-center gap-x-2.5 p-3 text-sm font-semibold leading-6 text-[#013894]"
                                        >
                    ...
                    {navItems.map((item)=>(
                        <a key={item.name} href={item.href} className="text-sm font-semibold leading-6 text-white">{item.name}</a>
                    ))}
                </Popover.Group>
            ...
            >
                <div className="fixed inset-0 z-10">
                    <Dialog.Panel className="fixed inset-y-0 right-0 z-10 w-full overflow-y-auto bg-[#013894] px-6 py-6 sm:max-w-sm sm:right-1 sm:ring-gray-900/10">
                        ...
                        <div className="mt-6 flow-root">
                            <div className="-m-6 divide-y divide-gray-500/10">
                                <div className="space-y-2 p-6">
                                    <Disclosure as="div" className="-mx-3">
                                        ...
                                        <Disclosure.Panel className="mt-2 space-y-2">
                                            {[...products, ...callsToAction].map((item)=>(
                                                <Disclosure.Button className="block rounded-bg items-center py-2 pl-6 pr-3 text-sm font-semibold leading-7 text-white hover:bg-blue-800">
                                                    {item.name}
                                                </Disclosure.Button>
                                            ))}
                                        </Disclosure.Panel>
                                    ...
                                    {navItems.map((item)=>(
                                        <a key={item.name} href={item.href} className="-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-white">{item.name}</a>
                                    ))}
                                </div>
                                <div className="p-6">
                                    <a className="-mx-3 block rounded-lg px-3 p-2 text-base font-semibold leading-7 text-white">
                                        Log In
                                    </a>
                                </div>
                            </div>
